<?php
/*******************************************************************************************
 *           Author    : Abdulmalik Mamirov                                                *
 *           Contact   : abdu98malik@mail.ru                                               *
 *******************************************************************************************/
...
class DB {
	...
	public function DB($Host="localhost", $DB="", $User="root", $Pass="root", $pesist=0, $PgPort=5432) {
		$this->dbname = $DB;
		$this->host = $Host;
		$this->username = $User;
		$this->password = $Pass;
	...
	public function open() {
		...
		if (!$this->dbconnect) $this->error = "Cannot connect to database {$this->dbname}";
	}
	...
	public function query($sql) {
		...
		if (!$this->result) die("Error in SQL query: " . $this->error);
